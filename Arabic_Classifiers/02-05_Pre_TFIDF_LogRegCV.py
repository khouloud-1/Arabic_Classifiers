...
# The solver parameter of LogisticRegressionCV is the algorithm to use in the optimization problem. By default it's 'lbfgs'
# The default cross-validation generator used is Stratified K-Folds. So the cv parameter of LogisticRegressionCV is the number of folds used.
clf = LogisticRegressionCV(solver='liblinear', cv = 5).fit(Xtrain, np.ravel(Ytrain))

#######################################

end = time.time()

############################### LogisticRegressionCV classifier evaluation #####################################
...
print('\nTime for preprocessing, TFIDF features extraction and LogisticRegressionCV training: ',(end-start),' sec')
print('\nFor TFIDF and LogisticRegressionCV, f1-score =                 '+str(fs))
