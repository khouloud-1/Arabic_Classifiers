...
############### NGRAMS Function with N = 2, i.e. 2-grams. You can change the N value...
...
with open('Used_Corpus/arabic_dataset_classifiction_100.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        line =  get_ngrams(clean_text(row['text']), N) 
...
print('\nNumber of features: '+ str(tf_mat_tr.shape[1]-1) + ' '+str(N)+'grams')
print('\nTime for preprocessing, '+str(N)+'gram-BOW features extraction and AdaBoostClassifier training: ',(end-start),' sec')
print('\nFor '+str(N)+'gram-BOW and AdaBoostClassifier, f1-score =                 '+str(fs))
...
tf_x.append(dictionary.doc2bow(get_ngrams(clean_text(x), N)))
