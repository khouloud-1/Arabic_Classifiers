...
#tfidf_mat = matutils.corpus2csc(bow_corpus)

#print("\n****** TFIDF matrix:  rows for terms in the lexicon and columns for lines (documents) in the corpus  ******")

tfidf_mat = np.vstack([tfidf_mat, Y])
#pprint(tfidf_mat[:, :])


tfidf_mat_tr = np.transpose(tfidf_mat[:, :])

#print("\n****** TFIDF matrix:  rows for documents and columns for terms. the last column is for Y  ******")
#pprint(tfidf_mat_tr[:, :])
...
XYtrain, XYtest = train_test_split(tfidf_mat_tr, test_size=0.3, train_size=0.7, shuffle=True)
...
# The base_estimator parameter of AdaBoostClassifier indicates the base estimator from which the boosted ensemble is built. If None, then the base estimator is DecisionTreeClassifier initialized with max_depth=1.
# The n_estimators parameter of AdaBoostClassifier indicates the maximum number of estimators at which boosting is terminated. By default it’s 50.
# The algorithm parameter of AdaBoostClassifier indicates the boosting algorithm. By default it’s 'SAMME.R'.
...
print('\nCorpus size (dataset size): '+ str(tfidf_mat_tr.shape[0]) + ' documents')
print('\nNumber of features: '+ str(tfidf_mat_tr.shape[1]-1) + ' terms')
print('\nTime for preprocessing, TFIDF features extraction and AdaBoostClassifier training: ',(end-start),' sec')
print('\nFor TFIDF and AdaBoostClassifier, f1-score =                 '+str(fs))
...
tfidf_x.append(tfidf[dictionary.doc2bow(clean_text(x).split())])

x_vec = matutils.corpus2dense(tfidf_x, num_terms=len(dictionary.token2id))
